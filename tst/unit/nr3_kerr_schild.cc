...
using pangu::nr::BuildSchwarzschildKerrSchild;
using pangu::nr::ComputeADMConstraints;
using pangu::nr::ConstraintComponent;
using pangu::nr::Index;
using pangu::nr::SchwarzschildKerrSchildData;

KOKKOS_INLINE_FUNCTION Real ConstraintError(const SchwarzschildKerrSchildData& data,
...
int main(int argc, char** argv) {
  ...
  const Real coordinates[5][3] = {
      {3.0, 4.0, 12.0}, {2.0, 0.0, 0.0}, {-2.0, 1.0, 0.0}, {0.0, 0.0, 4.0}, {1.0, -2.0, 3.0}};
  Kokkos::View<Real[5]> errors("NR-3 Kerr-Schild errors");
  Kokkos::View<int[5]> valid("NR-3 Kerr-Schild valid");
  Kokkos::parallel_for(
      "NR-3 Kerr-Schild initial data", 5, KOKKOS_LAMBDA(const int point) {
        SchwarzschildKerrSchildData data{};
        const bool ok = BuildSchwarzschildKerrSchild(coordinates[point][0], coordinates[point][1],
                                                     coordinates[point][2], 1.0, data);
        valid(point) = ok ? 1 : 0;
  ...
  const Real tolerance = 8192.0 * std::numeric_limits<Real>::epsilon();
  if (!all_valid || !std::isfinite(maximum) || maximum > tolerance) {
    std::cerr << "NR-3 Kerr-Schild initial-data failure: error=" << maximum
              << " tolerance=" << tolerance << " valid=" << all_valid << '\n';
    return 1;
  }
  std::cout << "NR-3 Kerr-Schild initial-data PASS: max constraint=" << maximum
            << " tolerance=" << tolerance << '\n';
  return 0;
}
