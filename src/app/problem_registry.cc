...
struct ProblemDefinition {
  ...
  void (*after_loop)(parthenon::Mesh*, parthenon::ParameterInput*, parthenon::SimTime&);
...
const std::map<std::string, ProblemDefinition>& Registry() {
  static std::map<std::string, ProblemDefinition> registry{
      {"hydro_advection",
       {pgen::HydroProblem, pgen::HydroAfterLoop, hydro::CalculateFluxesBlockTask,
        hydro::ApplyFirstOrderFluxCorrectionBlockTask, hydro::ApplySourcesBlockTask,
        hydro::Initialize}},
      {"sod",
       {pgen::HydroProblem, pgen::HydroAfterLoop, hydro::CalculateFluxesBlockTask,
        hydro::ApplyFirstOrderFluxCorrectionBlockTask, hydro::ApplySourcesBlockTask,
      ...
      {"mhd_shock",
       {pgen::MHDProblem, pgen::MHDAfterLoop, mhd::CalculateFluxesBlockTask,
        mhd::ApplyFirstOrderFluxCorrectionBlockTask, mhd::ApplySourcesBlockTask, mhd::Initialize}},
      {"brio_wu",
       {pgen::MHDProblem, pgen::MHDAfterLoop, mhd::CalculateFluxesBlockTask,
        mhd::ApplyFirstOrderFluxCorrectionBlockTask, mhd::ApplySourcesBlockTask, mhd::Initialize}},
      {"sr_mhd_shock",
  ...
  [](auto synchronized, auto& registry) {
    if constexpr (decltype(synchronized)::value) {
      static const bool inserted_sync_grhd_shock =
          registry
              .emplace("sync_grhd_shock",
                       ProblemDefinition{pgen::SyncGRHDProblem, pgen::HydroAfterLoop,
                                         hydro::CalculateFluxesBlockTask,
                                         hydro::ApplyFirstOrderFluxCorrectionBlockTask,
      ...
      static const bool inserted_sync_grhd_tov =
          registry
              .emplace("sync_grhd_tov",
                       ProblemDefinition{
                           pgen::SyncGRHDTOV, nullptr, hydro::CalculateFluxesBlockTask,
                           hydro::ApplyFirstOrderFluxCorrectionBlockTask,
                           hydro::ApplySourcesBlockTask, nr::Initialize, hydro::Initialize})
              .second;
      static const bool inserted_sync_grmhd_linear_wave =
          registry
              .emplace(
                  "sync_grmhd_linear_wave",
                  ProblemDefinition{pgen::SyncGRMHDProblem, nullptr, mhd::CalculateFluxesBlockTask,
                                    mhd::ApplyFirstOrderFluxCorrectionBlockTask,
                                    mhd::ApplySourcesBlockTask, nr::Initialize, mhd::Initialize})
              .second;
      static const bool inserted_sync_grmhd_tov =
          registry
              .emplace("sync_grmhd_tov",
                       ProblemDefinition{pgen::SyncGRMHDTOV, nullptr, mhd::CalculateFluxesBlockTask,
                                         mhd::ApplyFirstOrderFluxCorrectionBlockTask,
      ...
      static const bool inserted_sync_grmhd_boosted_puncture =
          registry
              .emplace("sync_grmhd_boosted_puncture",
                       ProblemDefinition{
                           pgen::SyncGRMHDBoostedPuncture, nullptr, mhd::CalculateFluxesBlockTask,
                           mhd::ApplyFirstOrderFluxCorrectionBlockTask, mhd::ApplySourcesBlockTask,
                           nr::Initialize, mhd::Initialize})
              .second;
      static const bool inserted_minkowski =
          registry
              .emplace("nr_minkowski", ProblemDefinition{pgen::NumericalRelativityMinkowski,
      ...
      static const bool inserted_schwarzschild =
          registry
              .emplace("nr_schwarzschild",
                       ProblemDefinition{pgen::NumericalRelativitySchwarzschild, nullptr, nullptr,
                                         nullptr, nullptr, nr::Initialize})
              .second;
      static const bool inserted_schwarzschild_ks =
          registry
              .emplace("nr_schwarzschild_ks",
                       ProblemDefinition{pgen::NumericalRelativitySchwarzschildKerrSchild, nullptr,
                                         nullptr, nullptr, nullptr, nr::Initialize})
              .second;
      static const bool inserted_boosted_puncture =
          registry
              .emplace("nr_boosted_puncture",
                       ProblemDefinition{pgen::NumericalRelativityBoostedPuncture, nullptr, nullptr,
                                         nullptr, nullptr, nr::Initialize})
              .second;
      ...
      static const bool inserted_two_punctures =
          registry
              .emplace("nr_two_punctures",
                       ProblemDefinition{pgen::NumericalRelativityTwoPunctures, nullptr, nullptr,
                                         nullptr, nullptr, nr::Initialize})
              .second;
#endif
      static const bool inserted_linear_wave =
          registry
              .emplace("nr_linear_wave",
                       ProblemDefinition{pgen::NumericalRelativityLinearWave,
                                         pgen::NumericalRelativityLinearWaveAfterLoop, nullptr,
                                         nullptr, nullptr, nr::Initialize})
              .second;
      static const bool inserted_gauge_wave =
          registry
              .emplace("nr_gauge_wave",
                       ProblemDefinition{pgen::NumericalRelativityGaugeWave,
                                         pgen::NumericalRelativityGaugeWaveAfterLoop, nullptr,
                                         nullptr, nullptr, nr::Initialize})
              .second;
      static const bool inserted_robust_stability =
          registry
              .emplace("nr_robust_stability",
                       ProblemDefinition{pgen::NumericalRelativityRobustStability,
                                         pgen::NumericalRelativityRobustStabilityAfterLoop, nullptr,
                                         nullptr, nullptr, nr::Initialize})
              .second;
      [[maybe_unused]] const bool all_registered =
          inserted_minkowski && inserted_sync_grhd_shock && inserted_sync_grhd_linear_wave &&
          inserted_sync_grhd_tov && inserted_sync_grmhd_tov &&
          inserted_sync_grmhd_boosted_puncture && inserted_sync_grmhd_linear_wave &&
          inserted_schwarzschild && inserted_schwarzschild_ks && inserted_boosted_puncture &&
#if PANGU_ENABLE_TWOPUNCTURES
          inserted_two_punctures &&
#endif
          inserted_linear_wave && inserted_gauge_wave && inserted_robust_stability;
    }
  }(std::bool_constant<geometry::ConfiguredGeometryIsSynchronized()>{}, registry);
  return registry;
}

template <bool Synchronized>
void ConfigureSynchronizedProblem([[maybe_unused]] const std::string& problem_id,
...
void ConfigureRadiationBoundaries([[maybe_unused]] const std::string& problem_id,
                                  [[maybe_unused]] parthenon::ApplicationInput* app_input) {
...
void ConfigureProblem(const std::string& problem_id, parthenon::ApplicationInput* app_input) {
  ...
  const bool synchronized_geometry = geometry::ConfiguredGeometryIsSynchronized();
  ...
  const bool synchronized_problem = numerical_relativity_problem || synchronized_matter_problem;
  PARTHENON_REQUIRE(
      ...
      synchronized_problem
          ...
          : "a z4c+sync build accepts only numerical-relativity or synchronized-matter "
            "ProblemTypes");
  ConfigureSynchronizedProblem<geometry::ConfiguredGeometryIsSynchronized()>(problem_id, app_input);
  ...
  if (problem_id == "gr_chakrabarti_torus" || problem_id == "gr_chakrabarti_torus_sane") {
...
BlockFluxCorrectionTask GetBlockFluxCorrectionTask(const std::string& problem_id) {
  const auto definition = Registry().find(problem_id);
  if (definition == Registry().end()) {
    PARTHENON_FAIL("No first-order correction registry entry for problem_id='" + problem_id + "'");
  }
...
PackageInitializer GetPackageInitializer(const std::string& problem_id) {
  const auto definition = Registry().find(problem_id);
  if (definition == Registry().end()) {
    PARTHENON_FAIL("No package initializer registered for problem_id='" + problem_id + "'");
  }
...
PackageInitializer GetCoupledPackageInitializer(const std::string& problem_id) {
  const auto definition = Registry().find(problem_id);
  if (definition == Registry().end()) {
    PARTHENON_FAIL("No coupled package initializer registered for problem_id='" + problem_id + "'");
  }
