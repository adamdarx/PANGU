...
template <class Pack> struct InverseMetricAccessor {
  ...
  KOKKOS_INLINE_FUNCTION Real operator()(const int dk, const int dj, const int di) const {
    ...
    for (int component = 0; component < 6; ++component)
      metric[component] = z4c(nr::Index(nr::Z4cComponent::gxx) + component, k + dk, j + dj, i + di);
...
template <int Order>
void InitializeGaugeWaveGamma(MeshBlock* block, const parthenon::VariablePack<Real>& z4c) {
  ...
  block->par_for(
      "PANGU NR gauge-wave conformal connection", kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        for (int upper = 0; upper < 3; ++upper) {
          ...
          for (int derivative = 0; derivative < 3; ++derivative) {
            ...
            gamma -= nr::fd::First<Order>(derivative, inverse_spacing[derivative], accessor);
          }
...
KOKKOS_INLINE_FUNCTION Real DeterministicNoise(const std::uint64_t plane,
  ...
  std::uint64_t value =
      0x9e3779b97f4a7c15ULL * (plane + 1ULL) ^ 0xbf58476d1ce4e5b9ULL * (component + 1ULL);
...
void InitializeConstraintMask(MeshBlock* block, const Real mask_radius = 0.0) {
  ...
  block->par_for(
      ...
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        const bool masked = mask_radius > 0.0 && x * x + y * y + z * z <= mask_radius_squared;
...
template <int Order> void InitializeSyncGRHDTOV(MeshBlock* block, ParameterInput* pin) {
  InitializeConstraintMask(block);
  auto data = block->meshblock_data.Get();
  ...
  PARTHENON_REQUIRE(fabs(eos.gamma - profile.gamma) <= 16.0 * std::numeric_limits<Real>::epsilon(),
                    "TOV polytropic gamma must match hydro/gamma");
  PARTHENON_REQUIRE(eos.density_floor <= profile.density_floor,
                    "hydro density floor must not exceed the TOV atmosphere density");
  PARTHENON_REQUIRE(eos.pressure_floor <= profile.pressure_floor,
                    "hydro pressure floor must not exceed the TOV atmosphere pressure");
  PARTHENON_REQUIRE(fabs(radial_velocity) < 1.0,
  ...
  const auto coordinates = block->coords;
  block->par_for(
      "PANGU SYNC TOV ADM and primitive initial data", kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        const Real x = coordinates.Xc<X1DIR>(k, j, i);
        const Real y = coordinates.Xc<X2DIR>(k, j, i);
        const Real z = coordinates.Xc<X3DIR>(k, j, i);
        ...
        for (int derivative = 0; derivative < 3; ++derivative) {
          for (int component = 0; component < 6; ++component) {
            const TOVMetricAccessor accessor{profile, x, y, z, spacing[0], spacing[1], spacing[2],
                                             component};
            derivatives.dmetric[derivative][component] =
                nr::fd::First<Order>(derivative, 1.0 / spacing[derivative], accessor);
          }
        }
        nr::Z4cState converted{};
        if (!nr::ConvertADMToZ4c(adm_state, derivatives, options.chi_psi_power, converted))
        ...
        primitive(hydro::IPR, k, j, i) = fmax(point.pressure, eos.pressure_floor);
        for (int axis = 0; axis < 3; ++axis)
          primitive(hydro::IV1 + axis, k, j, i) = 0.0;
        if (point.inside && radius > 0.0 && radial_velocity != 0.0) {
          const Real fraction = point.schwarzschild_radius / profile.surface_radius;
          ...
          const Real projected_radial_velocity =
              0.5 * radial_velocity * (3.0 * fraction - fraction * fraction * fraction);
  ...
  block->par_for(
      "PANGU SYNC TOV Valencia initial data", kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
...
KOKKOS_INLINE_FUNCTION Real TOVVectorPotential(const nr::TOVProfile& profile,
  ...
  const Real density_weight =
      pow(fmax(1.0 - point.density / profile.central_density, 0.0), magnetic_index);
...
template <int Order> void InitializeSyncGRMHDTOV(MeshBlock* block, ParameterInput* pin) {
  InitializeConstraintMask(block);
  auto data = block->meshblock_data.Get();
  const auto nr_package = block->packages.Get("numerical_relativity");
  ...
  const Real radial_velocity = pin->GetOrAddReal("problem", "v_pert", 0.0);
  const Real magnetic_normalization = pin->GetOrAddReal("problem", "b_norm", 1.0);
  ...
  PARTHENON_REQUIRE(pressure_cut >= 0.0, "TOV magnetic pressure cut must be nonnegative");
  PARTHENON_REQUIRE(magnetic_index >= 0.0, "TOV magnetic index must be nonnegative");
  ...
  block->par_for(
      "PANGU SYNC magnetized TOV ADM and primitive initial data", kb.s, kb.e, jb.s,
      jb.e, ib.s, ib.e, KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        nr::Z4cState converted{};
        if (!nr::ConvertADMToZ4c(adm_state, derivatives, options.chi_psi_power, converted))
          Kokkos::abort("failed to convert magnetized TOV ADM initial data to Z4c");
        for (int component = 0; component < nr::kZ4cComponents; ++component)
          z4c(component, k, j, i) = converted.values[component];
  ...
  block->par_for(
      "PANGU SYNC magnetized TOV vector potential", interior_kb.s, interior_kb.e + 1,
      interior_jb.s, interior_jb.e + 1, interior_ib.s, interior_ib.e + 1,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        const int ii = i - interior_ib.s;
        const int jj = j - interior_jb.s;
        const int kk = k - interior_kb.s;
        potential(0, kk, jj, ii) = magnetic_normalization * TOVVectorPotential<0>(
            profile, pressure_cut, magnetic_index, coordinates.Xc<X1DIR>(k, j, i),
            coordinates.Xf<X2DIR>(k, j, i), coordinates.Xf<X3DIR>(k, j, i));
        potential(1, kk, jj, ii) = magnetic_normalization * TOVVectorPotential<1>(
            profile, pressure_cut, magnetic_index, coordinates.Xf<X1DIR>(k, j, i),
            coordinates.Xc<X2DIR>(k, j, i), coordinates.Xf<X3DIR>(k, j, i));
        potential(2, kk, jj, ii) = 0.0;
      });
  block->par_for(
      "PANGU SYNC magnetized TOV B1", interior_kb.s, interior_kb.e, interior_jb.s,
      interior_jb.e, interior_ib.s, interior_ib.e + 1,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        const int ii = i - interior_ib.s;
        const int jj = j - interior_jb.s;
        const int kk = k - interior_kb.s;
  ...
  block->par_for(
      "PANGU SYNC magnetized TOV B2", interior_kb.s, interior_kb.e, interior_jb.s,
      interior_jb.e + 1, interior_ib.s, interior_ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        const int ii = i - interior_ib.s;
        const int jj = j - interior_jb.s;
        const int kk = k - interior_kb.s;
  ...
  block->par_for(
      "PANGU SYNC magnetized TOV cell magnetic field", interior_kb.s, interior_kb.e,
      interior_jb.s, interior_jb.e, interior_ib.s, interior_ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        bcell(0, k, j, i) =
...
#if PANGU_ENABLE_TWOPUNCTURES
struct TwoPunctureSolution {
  ini_data* data = nullptr;
...
char* TwoPunctureKey(const char* key) { return const_cast<char*>(key); }

TwoPunctureSolution& GetTwoPunctureSolution(ParameterInput* pin) {
  static TwoPunctureSolution solution;
  if (solution.data != nullptr)
    return solution;

  TwoPunctures_params_set_default();
  const auto set_real = [&](const char* key, const Real fallback) {
    TwoPunctures_params_set_Real(TwoPunctureKey(key), pin->GetOrAddReal("problem", key, fallback));
  };
  const auto set_integer = [&](const char* key, const int fallback) {
    TwoPunctures_params_set_Int(TwoPunctureKey(key),
                                pin->GetOrAddInteger("problem", key, fallback));
  };
  const auto set_boolean = [&](const char* key, const bool fallback) {
    TwoPunctures_params_set_Boolean(TwoPunctureKey(key),
                                    pin->GetOrAddBoolean("problem", key, fallback));
  };

  set_boolean("verbose", false);
  set_real("par_b", 3.257);
  set_real("par_m_plus", 0.483);
  set_real("par_m_minus", 0.483);
  set_real("target_M_plus", 0.505);
  set_real("target_M_minus", 0.505);
  set_real("par_P_plus1", 0.0);
  set_real("par_P_plus2", -0.133);
  set_real("par_P_plus3", 0.0);
  set_real("par_P_minus1", 0.0);
  set_real("par_P_minus2", 0.133);
  set_real("par_P_minus3", 0.0);
  set_real("par_S_plus1", 0.0);
  set_real("par_S_plus2", 0.0);
  set_real("par_S_plus3", 0.0);
  set_real("par_S_minus1", 0.0);
  set_real("par_S_minus2", 0.0);
  set_real("par_S_minus3", 0.0);
  set_real("center_offset1", 0.0);
  set_real("center_offset2", 0.0);
  set_real("center_offset3", 0.0);
  set_boolean("give_bare_mass", true);
  set_integer("grid_setup_method", 0);
  set_integer("npoints_A", 30);
  set_integer("npoints_B", 30);
  set_integer("npoints_phi", 16);
  set_real("Newton_tol", 1.0e-10);
  set_integer("Newton_maxit", 5);
  set_real("TP_epsilon", 0.0);
  set_real("TP_Tiny", 0.0);
  set_real("TP_Extend_Radius", 0.0);
  set_real("adm_tol", 1.0e-10);
  set_boolean("do_residuum_debug_output", false);
  set_boolean("solve_momentum_constraint", false);
  set_real("initial_lapse_psi_exponent", -2.0);
  set_boolean("swap_xz", false);

  solution.data = TwoPunctures_make_initial_data();
  PARTHENON_REQUIRE(solution.data != nullptr, "TwoPuncturesC failed to construct initial data");
  if (Globals::my_rank == 0)
    std::cout << "PANGU TwoPuncturesC constraint-satisfying initial data ready.\n";
  return solution;
}

void InitializeTwoPunctureConstraintMask(MeshBlock* block, ParameterInput* pin) {
  const Real radius = pin->GetOrAddReal("problem", "constraint_mask_radius", 0.75);
  const Real separation = pin->GetOrAddReal("problem", "par_b", 3.257);
  const Real offset[3] = {pin->GetOrAddReal("problem", "center_offset1", 0.0),
  ...
  PARTHENON_REQUIRE(radius >= 0.0, "NR two-puncture constraint_mask_radius must be nonnegative");
  Real plus[3] = {offset[0] + separation, offset[1], offset[2]};
  Real minus[3] = {offset[0] - separation, offset[1], offset[2]};
  if (swap_xz) {
    plus[0] = offset[0];
    minus[0] = offset[0];
    plus[2] = offset[2] + separation;
    minus[2] = offset[2] - separation;
  }
  auto data = block->meshblock_data.Get();
  const auto mask = data->PackVariables(std::vector<std::string>{"nr.constraint_mask"});
  const auto ib = block->cellbounds.GetBoundsI(IndexDomain::entire);
  ...
  block->par_for(
      "PANGU NR two-puncture constraint mask", kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        const Real point[3] = {coordinates.Xc<X1DIR>(k, j, i), coordinates.Xc<X2DIR>(k, j, i),
                               coordinates.Xc<X3DIR>(k, j, i)};
        Real distance_plus = 0.0;
        Real distance_minus = 0.0;
        for (int axis = 0; axis < 3; ++axis) {
          distance_plus += (point[axis] - plus[axis]) * (point[axis] - plus[axis]);
          distance_minus += (point[axis] - minus[axis]) * (point[axis] - minus[axis]);
        }
        mask(0, k, j, i) =
            radius > 0.0 && (distance_plus <= radius_squared || distance_minus <= radius_squared)
                ? 0.0
                : 1.0;
      });
}
...
KOKKOS_INLINE_FUNCTION int ReflectionSign(const int component, const int direction) {
  ...
  if (component >= nr::Index(nr::Z4cComponent::gxx) &&
    ...
    for (int candidate_first = 0; candidate_first < 3; ++candidate_first) {
      for (int candidate_second = candidate_first; candidate_second < 3; ++candidate_second) {
        if (nr::SpatialSymmetricComponent(candidate_first, candidate_second) == spatial) {
...
void NumericalRelativityReflectingBoundary(std::shared_ptr<MeshBlockData<Real>>& data,
                                           const bool coarse) {
  ...
  const auto z4c = data->PackVariables(std::vector<std::string>{"nr.z4c"}, coarse);
  int il = ei.s;
  int iu = ei.e;
  int jl = ej.s;
  int ju = ej.e;
  ...
  block->par_for(
      ...
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        for (int component = 0; component < nr::kZ4cComponents; ++component)
          z4c(component, k, j, i) = static_cast<Real>(ReflectionSign(component, Direction)) *
                                    z4c(component, reflected_k, reflected_j, reflected_i);
      });
}

template <int Direction, bool Inner>
void NumericalRelativityExtrapolationBoundary(std::shared_ptr<MeshBlockData<Real>>& data,
                                              const bool coarse) {
  ...
  auto block = data->GetBlockPointer();
  const int order =
      block->packages.Get("numerical_relativity")->Param<int>("boundary_extrapolation_order");
  if (order == 0)
    return;
  if (order == 2) {
  ...
  block->par_for(
      ...
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        const int ghost_coordinate = Direction == 2 ? k : (Direction == 1 ? j : i);
        ...
        for (int component = 0; component < nr::kZ4cComponents; ++component) {
          ...
          if (order == 0) {
            ...
            const Real f1 = z4c(component, sk + offk, sj + offj, si + offi);
            const Real f2 = z4c(component, sk + 2 * offk, sj + 2 * offj, si + 2 * offi);
            z4c(component, k, j, i) = 0.5 * (f0 * (1.0 + delta) * (2.0 + delta) +
                                             delta * (f2 + delta * f2 - 2.0 * f1 * (2.0 + delta)));
          } else {
            const Real f1 = z4c(component, sk + offk, sj + offj, si + offi);
            const Real f2 = z4c(component, sk + 2 * offk, sj + 2 * offj, si + 2 * offi);
            const Real f3 = z4c(component, sk + 3 * offk, sj + 3 * offj, si + 3 * offi);
            z4c(component, k, j, i) =
                (-3.0 * f1 * delta * (2.0 + delta) * (3.0 + delta) +
                 f0 * (1.0 + delta) * (2.0 + delta) * (3.0 + delta) +
                 delta * (1.0 + delta) * (-f3 * (2.0 + delta) + 3.0 * f2 * (3.0 + delta))) /
                6.0;
          }
        }
      });
...
void SyncGRHDProblem(MeshBlock* block, ParameterInput* pin) {
  const auto nr_package = block->packages.Get("numerical_relativity");
  const auto hydro_package = block->packages.Get("hydro");
  const auto matter_source = nr_package->Param<std::string>("matter_source_name");
  PARTHENON_REQUIRE(matter_source == "grhd" || matter_source == "zero",
...
void SyncGRMHDProblem(MeshBlock* block, ParameterInput* pin) {
  ...
  const auto mhd_package = block->packages.Get("mhd");
  const auto matter_source = nr_package->Param<std::string>("matter_source_name");
  PARTHENON_REQUIRE(matter_source == "grmhd" || matter_source == "zero",
  ...
  PARTHENON_REQUIRE(wave_flag >= 0 && wave_flag <= 6,
  ...
  PARTHENON_REQUIRE(aligned_directions <= 1,
  ...
  const auto wave = relativistic_mhd_wave::Build(
      pin->GetOrAddReal("problem", "dens", 4.0),
      ...
      pin->GetOrAddReal("problem", "bz0", -1.2), eos.gamma, amplitude, wave_flag);
  ...
  if (block->gid == 0)
    std::cout << std::setprecision(17)
              ...
              << " speed=" << wave.wave_speed << " wavelength="
              << wave_geometry.wavelength << " amplitude=" << amplitude << '\n';
  block->par_for(
      ...
      ib.e, KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        const Real sinusoid = sin(
            wave_number * relativistic_mhd_wave::WaveCoordinate(
  ...
  if (along_x1) {
    ...
    block->par_for(
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
          ...
          const Real potential_left =
              -wave.magnetic[1] * left +
              wave.delta_transverse_magnetic[0] / wave_number *
                  cos(wave_number * left);
          const Real potential_right =
              -wave.magnetic[1] * right +
              wave.delta_transverse_magnetic[0] / wave_number *
    ...
    block->par_for(
        "PANGU sync GRMHD B3 initialization", kb.s,
        kb.e + (ndim >= 3), jb.s, jb.e, ib.s, ib.e,
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
          const Real left = coordinates.Xf<X1DIR>(k, j, i);
          const Real right = coordinates.Xf<X1DIR>(k, j, i + 1);
          const Real potential_left =
              wave.magnetic[2] * left -
              wave.delta_transverse_magnetic[1] / wave_number *
                  cos(wave_number * left);
          const Real potential_right =
              wave.magnetic[2] * right -
              wave.delta_transverse_magnetic[1] / wave_number *
                  cos(wave_number * right);
          bface(2, 0, 0, 0, k, j, i) =
              (potential_right - potential_left) / (right - left);
    ...
    ParArray4DRaw<Real> potential(
        "PANGU sync GRMHD vector potential", 3, potential_nk,
        potential_nj, potential_ni);
    block->par_for(
        "PANGU sync GRMHD vector potential", kb.s, kb.e + 1, jb.s,
        jb.e + 1, ib.s, ib.e + 1,
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
          const int ii = i - ib.s;
          const int jj = j - jb.s;
          ...
          if (a1_correct)
            a1 = 0.5 *
                 (relativistic_mhd_wave::VectorPotential<0>(
                      x1_center - 0.25 * dx1, x2_face, x3_face, wave, wave_geometry) +
                  relativistic_mhd_wave::VectorPotential<0>(
                      x1_center + 0.25 * dx1, x2_face, x3_face, wave, wave_geometry));
          ...
          if (a2_correct)
            a2 = 0.5 *
                 (relativistic_mhd_wave::VectorPotential<1>(
                      x1_face, x2_center - 0.25 * dx2, x3_face, wave, wave_geometry) +
                  relativistic_mhd_wave::VectorPotential<1>(
                      x1_face, x2_center + 0.25 * dx2, x3_face, wave, wave_geometry));
          ...
          if (a3_correct)
            a3 = 0.5 *
                 (relativistic_mhd_wave::VectorPotential<2>(
                      x1_face, x2_face, x3_center - 0.25 * dx3, wave, wave_geometry) +
                  relativistic_mhd_wave::VectorPotential<2>(
                      x1_face, x2_face, x3_center + 0.25 * dx3, wave, wave_geometry));
    ...
    block->par_for(
        "PANGU sync GRMHD diagonal B1 initialization", kb.s, kb.e,
        jb.s, jb.e, ib.s, ib.e + 1,
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
          const int ii = i - ib.s;
          const int jj = j - jb.s;
          const int kk = k - kb.s;
          bface(0, 0, 0, 0, k, j, i) =
              (potential(2, kk, jj + 1, ii) -
               potential(2, kk, jj, ii)) /
                  coordinates.Dxc<X2DIR>(k, j, i) -
              (potential(1, kk + 1, jj, ii) -
               potential(1, kk, jj, ii)) /
                  coordinates.Dxc<X3DIR>(k, j, i);
        });
    block->par_for(
        "PANGU sync GRMHD diagonal B2 initialization", kb.s, kb.e,
        jb.s, jb.e + 1, ib.s, ib.e,
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
          const int ii = i - ib.s;
          const int jj = j - jb.s;
          const int kk = k - kb.s;
          bface(1, 0, 0, 0, k, j, i) =
              (potential(0, kk + 1, jj, ii) -
               potential(0, kk, jj, ii)) /
                  coordinates.Dxc<X3DIR>(k, j, i) -
              (potential(2, kk, jj, ii + 1) -
               potential(2, kk, jj, ii)) /
                  coordinates.Dxc<X1DIR>(k, j, i);
        });
    block->par_for(
        "PANGU sync GRMHD diagonal B3 initialization", kb.s, kb.e + 1,
        jb.s, jb.e, ib.s, ib.e,
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
          const int ii = i - ib.s;
          const int jj = j - jb.s;
          const int kk = k - kb.s;
          bface(2, 0, 0, 0, k, j, i) =
              (potential(1, kk, jj, ii + 1) -
               potential(1, kk, jj, ii)) /
                  coordinates.Dxc<X1DIR>(k, j, i) -
              (potential(0, kk, jj + 1, ii) -
               potential(0, kk, jj, ii)) /
                  coordinates.Dxc<X2DIR>(k, j, i);
        });
  }
...
void SyncGRHDTOV(MeshBlock* block, ParameterInput* pin) {
  const auto nr_package = block->packages.Get("numerical_relativity");
  const auto hydro_package = block->packages.Get("hydro");
  PARTHENON_REQUIRE(nr_package->Param<std::string>("matter_source_name") == "grhd",
...
void SyncGRMHDTOV(MeshBlock* block, ParameterInput* pin) {
  const auto nr_package = block->packages.Get("numerical_relativity");
  const auto mhd_package = block->packages.Get("mhd");
  PARTHENON_REQUIRE(nr_package->Param<std::string>("matter_source_name") == "grmhd",
...
void NumericalRelativitySchwarzschild(MeshBlock* block, ParameterInput* pin) {
  const Real mask_radius = pin->GetOrAddReal("problem", "constraint_mask_radius", 0.0);
  ...
  const auto options =
      block->packages.Get("numerical_relativity")->Param<nr::Z4cOptions>("z4c_options");
  PARTHENON_REQUIRE(mass >= 0.0, "NR Schwarzschild mass must be nonnegative");
  block->par_for(
      "PANGU NR Schwarzschild isotropic initial data", kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        const bool valid = nr::BuildSchwarzschildIsotropic(
            ...
            coordinates.Xc<X3DIR>(k, j, i), mass, options.chi_psi_power, initial);
        for (int component = 0; component < nr::kZ4cComponents; ++component)
          z4c(component, k, j, i) = 0.0;
        if (valid) {
          const Real chi =
              pow(nr::rhs::SpatialDeterminant(initial.adm.metric), options.chi_psi_power / 12.0);
...
void NumericalRelativitySchwarzschildKerrSchild(MeshBlock* block, ParameterInput* pin) {
  const Real mask_radius = pin->GetOrAddReal("problem", "constraint_mask_radius", 0.0);
  PARTHENON_REQUIRE(mask_radius >= 0.0,
                    "NR Schwarzschild Kerr-Schild constraint_mask_radius must be nonnegative");
  InitializeConstraintMask(block, mask_radius);
  auto data = block->meshblock_data.Get();
  const auto z4c = data->PackVariables(std::vector<std::string>{"nr.z4c"});
  const auto ib = block->cellbounds.GetBoundsI(IndexDomain::entire);
  ...
  const auto options =
      block->packages.Get("numerical_relativity")->Param<nr::Z4cOptions>("z4c_options");
  PARTHENON_REQUIRE(mass >= 0.0, "NR Schwarzschild Kerr-Schild mass must be nonnegative");
  block->par_for(
      "PANGU NR Schwarzschild Kerr-Schild initial data", kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        nr::Z4cState converted{};
        const bool converted_ok = valid && nr::ConvertADMToZ4c(initial.adm, initial.derivatives,
                                                               options.chi_psi_power, converted);
        for (int component = 0; component < nr::kZ4cComponents; ++component)
          z4c(component, k, j, i) = converted_ok ? converted.values[component] : 0.0;
        if (!converted_ok) {
          z4c(nr::Index(nr::Z4cComponent::chi), k, j, i) = 1.0;
          z4c(nr::Index(nr::Z4cComponent::gxx), k, j, i) = 1.0;
          z4c(nr::Index(nr::Z4cComponent::gyy), k, j, i) = 1.0;
...
void InitializeNumericalRelativityBoostedPuncture(MeshBlock* block, ParameterInput* pin,
                                                  const bool finalize_matter) {
  const Real mask_radius = pin->GetOrAddReal("problem", "constraint_mask_radius", 0.5);
  PARTHENON_REQUIRE(mask_radius >= 0.0,
  ...
  PARTHENON_REQUIRE(mass > 0.0, "NR boosted puncture punc_ADM_mass must be positive");
  PARTHENON_REQUIRE(std::fabs(velocity_y) <= 1.0e-14 && std::fabs(velocity_z) <= 1.0e-14,
                    "NR boosted puncture currently supports only an x1 boost");
  PARTHENON_REQUIRE(velocity_x * velocity_x < 1.0,
  ...
  const auto options =
      block->packages.Get("numerical_relativity")->Param<nr::Z4cOptions>("z4c_options");
  block->par_for(
      "PANGU NR boosted-puncture initial data", kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        nr::BoostedPunctureData initial{};
        const bool valid = nr::BuildBoostedPuncture(
            coordinates.Xc<X1DIR>(k, j, i) - center_x, coordinates.Xc<X2DIR>(k, j, i) - center_y,
            coordinates.Xc<X3DIR>(k, j, i) - center_z, mass, velocity_x, initial);
        for (int component = 0; component < nr::kZ4cComponents; ++component)
          z4c(component, k, j, i) = 0.0;
        if (!valid) {
          z4c(nr::Index(nr::Z4cComponent::chi), k, j, i) = 1.0;
        ...
        const Real determinant = nr::rhs::SpatialDeterminant(initial.adm.metric);
        const Real chi = pow(determinant, options.chi_psi_power / 12.0);
        Real inverse_metric[6]{};
        nr::rhs::SpatialInverse(1.0 / determinant, initial.adm.metric, inverse_metric);
        Real trace_k = 0.0;
        ...
        z4c(nr::Index(nr::Z4cComponent::alpha), k, j, i) = pow(chi, -2.0 / options.chi_psi_power);
        for (int axis = 0; axis < 3; ++axis)
          z4c(nr::Index(nr::Z4cComponent::betax) + axis, k, j, i) = initial.adm.shift[axis];
        for (int component = 0; component < 6; ++component) {
          const Real conformal_metric = chi * initial.adm.metric[component];
          z4c(nr::Index(nr::Z4cComponent::gxx) + component, k, j, i) = conformal_metric;
          z4c(nr::Index(nr::Z4cComponent::axx) + component, k, j, i) =
              chi *
              (initial.adm.extrinsic[component] - initial.adm.metric[component] * trace_k / 3.0);
        }
      });
...
void SyncGRMHDBoostedPuncture(MeshBlock* block, ParameterInput* pin) {
  ...
  const int ndim = block->pmy_mesh->ndim;
  const auto eos = pangu::eos::ReadRelativistic(*mhd_package);
  const Real density = pin->GetOrAddReal(
      "problem", "atmosphere_density", mhd_package->Param<Real>("puncture_density"));
  const Real pressure = pin->GetOrAddReal(
      "problem", "atmosphere_pressure", mhd_package->Param<Real>("puncture_pressure"));
  ...
  PARTHENON_REQUIRE(density >= eos.density_floor && pressure >= eos.pressure_floor,
                    "boosted-puncture atmosphere must respect MHD floors");
  block->par_for(
      "PANGU sync GRMHD boosted-puncture atmosphere", kb.s, kb.e, jb.s, jb.e, ib.s,
      ib.e, KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        primitive(mhd::IPR, k, j, i) =
            eos.InternalEnergyDensity(pressure);
        bcell(0, k, j, i) = b1;
        bcell(1, k, j, i) = b2;
        bcell(2, k, j, i) = b3;
        divb(0, k, j, i) = 0.0;
...
#if PANGU_ENABLE_TWOPUNCTURES
void NumericalRelativityTwoPunctures(MeshBlock* block, ParameterInput* pin) {
  InitializeTwoPunctureConstraintMask(block, pin);
  auto& solution = GetTwoPunctureSolution(pin);
  auto data = block->meshblock_data.Get();
  ...
  const auto jb = block->cellbounds.GetBoundsJ(IndexDomain::entire);
  const auto kb = block->cellbounds.GetBoundsK(IndexDomain::entire);
  const int nx = ib.e - ib.s + 1;
  const int ny = jb.e - jb.s + 1;
  const int nz = kb.e - kb.s + 1;
  const std::size_t cells = static_cast<std::size_t>(nx) * ny * nz;
  const auto coordinates = block->coords;

  std::vector<double> x(nx), y(ny), z(nz);
  for (int local = 0; local < nx; ++local)
    x[local] = coordinates.Xc<X1DIR>(kb.s, jb.s, ib.s + local);
  for (int local = 0; local < ny; ++local)
    y[local] = coordinates.Xc<X2DIR>(kb.s, jb.s + local, ib.s);
  for (int local = 0; local < nz; ++local)
    z[local] = coordinates.Xc<X3DIR>(kb.s + local, jb.s, ib.s);

  std::vector<double> lapse(cells), psi(cells);
  std::vector<double> gxx(cells), gxy(cells), gxz(cells), gyy(cells), gyz(cells), gzz(cells);
  std::vector<double> kxx(cells), kxy(cells), kxz(cells), kyy(cells), kyz(cells), kzz(cells);
  int minimum[3]{0, 0, 0};
  int maximum[3]{nx, ny, nz};
  int dimensions[3]{nx, ny, nz};
  TwoPunctures_Cartesian_interpolation(
      solution.data, minimum, maximum, dimensions, x.data(), y.data(), z.data(), lapse.data(),
      psi.data(), nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
      gxx.data(), gxy.data(), gxz.data(), gyy.data(), gyz.data(), gzz.data(), kxx.data(),
      kxy.data(), kxz.data(), kyy.data(), kyz.data(), kzz.data());

  const auto options =
      block->packages.Get("numerical_relativity")->Param<nr::Z4cOptions>("z4c_options");
  bool valid = true;
  for (int local_k = 0; local_k < nz; ++local_k) {
    for (int local_j = 0; local_j < ny; ++local_j) {
      for (int local_i = 0; local_i < nx; ++local_i) {
        const int k = kb.s + local_k;
        const int j = jb.s + local_j;
        const int i = ib.s + local_i;
        const std::size_t flat =
            static_cast<std::size_t>(local_i) +
            static_cast<std::size_t>(nx) * (local_j + static_cast<std::size_t>(ny) * local_k);
        const Real psi4 = static_cast<Real>(psi[flat] * psi[flat] * psi[flat] * psi[flat]);
        const Real metric[6] = {
            psi4 * static_cast<Real>(gxx[flat]), psi4 * static_cast<Real>(gxy[flat]),
            psi4 * static_cast<Real>(gxz[flat]), psi4 * static_cast<Real>(gyy[flat]),
            psi4 * static_cast<Real>(gyz[flat]), psi4 * static_cast<Real>(gzz[flat])};
        const Real extrinsic[6] = {static_cast<Real>(kxx[flat]), static_cast<Real>(kxy[flat]),
                                   static_cast<Real>(kxz[flat]), static_cast<Real>(kyy[flat]),
                                   static_cast<Real>(kyz[flat]), static_cast<Real>(kzz[flat])};
        const Real determinant = nr::rhs::SpatialDeterminant(metric);
        ...
        const Real chi = pow(determinant, options.chi_psi_power / 12.0);
        Real inverse_metric[6]{};
        nr::rhs::SpatialInverse(1.0 / determinant, metric, inverse_metric);
        Real trace_k = 0.0;
        for (int first = 0; first < 3; ++first)
          for (int second = 0; second < 3; ++second)
            trace_k += nr::rhs::SymmetricValue(inverse_metric, first, second) *
                       nr::rhs::SymmetricValue(extrinsic, first, second);
        ...
        for (int component = 0; component < 6; ++component) {
          host_z4c(nr::Index(nr::Z4cComponent::gxx) + component, k, j, i) = chi * metric[component];
          host_z4c(nr::Index(nr::Z4cComponent::axx) + component, k, j, i) =
              chi * (extrinsic[component] - metric[component] * trace_k / 3.0);
        }
      }
    }
  }
  PARTHENON_REQUIRE(valid, "TwoPuncturesC produced a non-finite or non-positive spatial metric");
...
void NumericalRelativityLinearWave(MeshBlock* block, ParameterInput* pin) {
  ...
  PARTHENON_REQUIRE(wave_number > 0.0, "NR linear wave requires a nonzero wavevector");
  const Real theta = atan2(sqrt(kx2 * kx2 + kx1 * kx1), kx3);
  const Real phi = atan2(kx1, kx2);
  const Real weights[6] = {
      -cos(theta) * cos(theta) * cos(2.0 * phi) - cos(phi) * cos(phi) * sin(theta) * sin(theta),
      -0.25 * (3.0 + cos(2.0 * theta)) * sin(2.0 * phi),
      -cos(theta) * sin(theta) * sin(phi),
      cos(theta) * cos(theta) * cos(2.0 * phi) - sin(theta) * sin(theta) * sin(phi) * sin(phi),
  ...
  block->par_for(
      ...
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        const Real phase =
            2.0 * M_PI *
            (kx1 * coordinates.Xc<X1DIR>(k, j, i) + kx2 * coordinates.Xc<X2DIR>(k, j, i) +
             kx3 * coordinates.Xc<X3DIR>(k, j, i));
        const Real sine = sin(phase);
        const Real cosine = wave_number * M_PI * cos(phase);
        for (int component = 0; component < 6; ++component) {
          const bool diagonal = component == nr::SpatialSymmetricComponent(0, 0) ||
                                component == nr::SpatialSymmetricComponent(1, 1) ||
                                component == nr::SpatialSymmetricComponent(2, 2);
          z4c(nr::Index(nr::Z4cComponent::gxx) + component, k, j, i) =
...
void NumericalRelativityGaugeWave(MeshBlock* block, ParameterInput* pin) {
  ...
  const auto options =
      block->packages.Get("numerical_relativity")->Param<nr::Z4cOptions>("z4c_options");
  PARTHENON_REQUIRE(amplitude > 0.0 && amplitude < 1.0, "NR gauge-wave amplitude must be in (0,1)");
  block->par_for(
      ...
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        z4c(nr::Index(nr::Z4cComponent::gxx), k, j, i) = conformal_scale * physical_gxx;
        ...
        z4c(nr::Index(nr::Z4cComponent::axx), k, j, i) =
            conformal_scale * physical_kxx - khat * conformal_scale * physical_gxx / 3.0;
        z4c(nr::Index(nr::Z4cComponent::ayy), k, j, i) = -khat * conformal_scale / 3.0;
        z4c(nr::Index(nr::Z4cComponent::azz), k, j, i) = -khat * conformal_scale / 3.0;
...
void NumericalRelativityRobustStability(MeshBlock* block, ParameterInput* pin) {
  ...
  block->par_for(
      "PANGU NR deterministic robust-stability initial data", kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        for (int component = 0; component < nr::kZ4cComponents; ++component)
          z4c(component, k, j, i) = 0.0;
        ...
        for (int component = 0; component < 6; ++component) {
          ...
          physical_metric[component] =
              (diagonal ? 1.0 : 0.0) + amplitude * DeterministicNoise(wrapped_plane, component);
          physical_k[component] = amplitude * DeterministicNoise(wrapped_plane, component + 6);
        }
        ...
        Real conformal_k[6]{};
        for (int component = 0; component < 6; ++component) {
          conformal_metric[component] = conformal_scale * physical_metric[component];
          conformal_k[component] = conformal_scale * physical_k[component];
        }
        Real inverse[6]{};
        const Real conformal_determinant = nr::rhs::SpatialDeterminant(conformal_metric);
        nr::rhs::SpatialInverse(1.0 / conformal_determinant, conformal_metric, inverse);
        Real khat = 0.0;
        for (int first = 0; first < 3; ++first) {
          for (int second = 0; second < 3; ++second) {
        ...
        z4c(nr::Index(nr::Z4cComponent::alpha), k, j, i) = 1.0;
        for (int component = 0; component < 6; ++component) {
          z4c(nr::Index(nr::Z4cComponent::gxx) + component, k, j, i) = conformal_metric[component];
          z4c(nr::Index(nr::Z4cComponent::axx) + component, k, j, i) =
...
void RegisterNumericalRelativityReflectingBoundaries(ApplicationInput* app_input) {
  app_input->RegisterBoundaryCondition(BoundaryFace::inner_x1, "nr_reflecting",
                                       NumericalRelativityReflectingBoundary<0, true>);
  app_input->RegisterBoundaryCondition(BoundaryFace::outer_x1, "nr_reflecting",
                                       NumericalRelativityReflectingBoundary<0, false>);
  app_input->RegisterBoundaryCondition(BoundaryFace::inner_x2, "nr_reflecting",
                                       NumericalRelativityReflectingBoundary<1, true>);
  app_input->RegisterBoundaryCondition(BoundaryFace::outer_x2, "nr_reflecting",
                                       NumericalRelativityReflectingBoundary<1, false>);
  app_input->RegisterBoundaryCondition(BoundaryFace::inner_x3, "nr_reflecting",
                                       NumericalRelativityReflectingBoundary<2, true>);
  app_input->RegisterBoundaryCondition(BoundaryFace::outer_x3, "nr_reflecting",
                                       NumericalRelativityReflectingBoundary<2, false>);
}

void RegisterNumericalRelativityExtrapolationBoundaries(ApplicationInput* app_input) {
  app_input->RegisterBoundaryCondition(BoundaryFace::inner_x1, "nr_extrapolate",
                                       NumericalRelativityExtrapolationBoundary<0, true>);
  app_input->RegisterBoundaryCondition(BoundaryFace::outer_x1, "nr_extrapolate",
                                       NumericalRelativityExtrapolationBoundary<0, false>);
  app_input->RegisterBoundaryCondition(BoundaryFace::inner_x2, "nr_extrapolate",
                                       NumericalRelativityExtrapolationBoundary<1, true>);
  app_input->RegisterBoundaryCondition(BoundaryFace::outer_x2, "nr_extrapolate",
                                       NumericalRelativityExtrapolationBoundary<1, false>);
  app_input->RegisterBoundaryCondition(BoundaryFace::inner_x3, "nr_extrapolate",
                                       NumericalRelativityExtrapolationBoundary<2, true>);
  app_input->RegisterBoundaryCondition(BoundaryFace::outer_x3, "nr_extrapolate",
...
void RegisterNumericalRelativityOutflowBoundaries(ApplicationInput* app_input) {
  ...
  app_input->RegisterBoundaryCondition(BoundaryFace::inner_x1, "nr_outflow",
                                       NumericalRelativityExtrapolationBoundary<0, true>);
  app_input->RegisterBoundaryCondition(BoundaryFace::outer_x1, "nr_outflow",
                                       NumericalRelativityExtrapolationBoundary<0, false>);
  app_input->RegisterBoundaryCondition(BoundaryFace::inner_x2, "nr_outflow",
                                       NumericalRelativityExtrapolationBoundary<1, true>);
  app_input->RegisterBoundaryCondition(BoundaryFace::outer_x2, "nr_outflow",
                                       NumericalRelativityExtrapolationBoundary<1, false>);
  app_input->RegisterBoundaryCondition(BoundaryFace::inner_x3, "nr_outflow",
                                       NumericalRelativityExtrapolationBoundary<2, true>);
  app_input->RegisterBoundaryCondition(BoundaryFace::outer_x3, "nr_outflow",
                                       NumericalRelativityExtrapolationBoundary<2, false>);
}
...
void NumericalRelativityAfterLoop(Mesh* mesh, ParameterInput*, SimTime& time) {
      ...
      for (int j = jb.s; j <= jb.e; ++j) {
        for (int i = ib.s; i <= ib.e; ++i) {
          for (int component = 0; component < nr::kZ4cComponents; ++component) {
            ...
            maximum = std::max(maximum, std::abs(z4c(component, k, j, i) - (unit ? 1.0 : 0.0)));
          }
          for (int component = 0; component < nr::kADMComponents; ++component) {
            ...
            maximum = std::max(maximum, std::abs(adm(component, k, j, i) - (unit ? 1.0 : 0.0)));
          }
        }
      }
    }
  ...
  PARTHENON_MPI_CHECK(
      MPI_Allreduce(&maximum, &global_maximum, 1, MPI_PARTHENON_REAL, MPI_MAX, MPI_COMM_WORLD));
  maximum = global_maximum;
#endif
  if (!vacuum) {
    PARTHENON_REQUIRE(std::isfinite(maximum),
  ...
  if (Globals::my_rank == 0) {
    std::cout << "NR-0 Minkowski evolution PASS: cycles=" << time.ncycle << " max error=" << maximum
              << '\n';
  }
}

void NumericalRelativityLinearWaveAfterLoop(Mesh* mesh, ParameterInput* pin, SimTime& time) {
  const Real amplitude = pin->GetReal("problem", "amplitude");
  const Real x1_length = mesh->mesh_size.xmax(X1DIR) - mesh->mesh_size.xmin(X1DIR);
  const Real x2_length = mesh->mesh_size.xmax(X2DIR) - mesh->mesh_size.xmin(X2DIR);
  const Real x3_length = mesh->mesh_size.xmax(X3DIR) - mesh->mesh_size.xmin(X3DIR);
  const Real kx1 = pin->GetOrAddReal("problem", "kx1", 1.0 / x1_length);
  const Real kx2 = pin->GetOrAddReal("problem", "kx2", 1.0 / x2_length);
  const Real kx3 = pin->GetOrAddReal("problem", "kx3", 1.0 / x3_length);
  const Real wave_number = sqrt(kx1 * kx1 + kx2 * kx2 + kx3 * kx3);
    ...
    for (int k = kb.s; k <= kb.e; ++k) {
      for (int j = jb.s; j <= jb.e; ++j) {
        for (int i = ib.s; i <= ib.e; ++i) {
          const Real phase =
              2.0 * M_PI *
              (kx1 * block->coords.Xc<X1DIR>(k, j, i) + kx2 * block->coords.Xc<X2DIR>(k, j, i) +
               kx3 * block->coords.Xc<X3DIR>(k, j, i) - wave_number * time.time);
          for (int component = 0; component < 6; ++component) {
            const bool diagonal = component == nr::SpatialSymmetricComponent(0, 0) ||
                                  component == nr::SpatialSymmetricComponent(1, 1) ||
                                  component == nr::SpatialSymmetricComponent(2, 2);
            const Real expected =
                (diagonal ? 1.0 : 0.0) + weights[component] * amplitude * sin(phase);
            const Real error =
                std::abs(z4c(nr::Index(nr::Z4cComponent::gxx) + component, k, j, i) - expected);
  ...
  PARTHENON_MPI_CHECK(
      MPI_Allreduce(error_sum, global_sum, 6, MPI_PARTHENON_REAL, MPI_SUM, MPI_COMM_WORLD));
  PARTHENON_MPI_CHECK(
      MPI_Allreduce(&maximum, &global_maximum, 1, MPI_PARTHENON_REAL, MPI_MAX, MPI_COMM_WORLD));
  PARTHENON_MPI_CHECK(
      MPI_Allreduce(&cells, &global_cells, 1, MPI_UINT64_T, MPI_SUM, MPI_COMM_WORLD));
  std::copy(std::begin(global_sum), std::end(global_sum), std::begin(error_sum));
  maximum = global_maximum;
  cells = global_cells;
#endif
  Real rms = 0.0;
...
void NumericalRelativityGaugeWaveAfterLoop(Mesh* mesh, ParameterInput* pin, SimTime& time) {
  const Real amplitude = pin->GetReal("problem", "amplitude");
    ...
    for (int k = kb.s; k <= kb.e; ++k) {
      for (int j = jb.s; j <= jb.e; ++j) {
        for (int i = ib.s; i <= ib.e; ++i) {
          ...
          const Real phase = 2.0 * M_PI * ((x - x1_min) / wavelength - time.time / wavelength);
          const Real h = amplitude * sin(phase);
          const Real physical_gxx = 1.0 - h;
          ...
          const Real errors[3] = {
              std::abs(adm(nr::Index(nr::ADMComponent::gxx), k, j, i) - physical_gxx),
              std::abs(adm(nr::Index(nr::ADMComponent::kxx), k, j, i) - expected_kxx),
              std::abs(z4c(nr::Index(nr::Z4cComponent::alpha), k, j, i) - expected_alpha)};
  ...
  PARTHENON_MPI_CHECK(
      MPI_Allreduce(&error_sum, &global_sum, 1, MPI_PARTHENON_REAL, MPI_SUM, MPI_COMM_WORLD));
  PARTHENON_MPI_CHECK(
      MPI_Allreduce(&maximum, &global_maximum, 1, MPI_PARTHENON_REAL, MPI_MAX, MPI_COMM_WORLD));
  PARTHENON_MPI_CHECK(
      MPI_Allreduce(&values, &global_values, 1, MPI_UINT64_T, MPI_SUM, MPI_COMM_WORLD));
  ...
  if (Globals::my_rank == 0)
    std::cout << "NR-2 gauge wave: cycles=" << time.ncycle << " L1=" << l1 << " Linf=" << maximum
              << '\n';
}

void NumericalRelativityRobustStabilityAfterLoop(Mesh* mesh, ParameterInput*, SimTime& time) {
  ...
  Real global_maximum = maximum;
  std::uint64_t global_values = values;
  PARTHENON_MPI_CHECK(MPI_Allreduce(&squared_sum, &global_squared_sum, 1, MPI_PARTHENON_REAL,
  ...
  if (Globals::my_rank == 0)
    std::cout << "NR-2 robust stability: cycles=" << time.ncycle << " RMS=" << rms
              << " Linf=" << maximum << '\n';
}
