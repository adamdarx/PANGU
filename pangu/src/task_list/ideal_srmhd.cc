...
// This file in the src/task_list module defines ideal_srmhd.cc responsibilities for the
...
Simulator::Simulator(ParameterInput *pin, ApplicationInput *app_in, Mesh *pm)
  ...
  pin->CheckDesired("core", "sigma_max");
  pin->CheckDesired("core", "lorentz_max");
}
...
TaskCollection Simulator::MakeTaskCollection(BlockList_t &blocks,
  ...
  for (int i = 0; i < blocks.size(); i++) {
    ...
    auto fix_prim = tl.AddTask(none, FixPrimitiveSRMHD, sc0);
    auto calc_cons = tl.AddTask(fix_prim, CalculateConservativeSRMHD, sc0);
    auto calc_flux = tl.AddTask(calc_cons, CalculateFluxesSRMHD, sc0);
  ...
  for (int i = 0; i < blocks.size(); i++) {
    ...
    auto trans_task = tl.AddTask(none, RecoverySRMHD, sc1);
  }
