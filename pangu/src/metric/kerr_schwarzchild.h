...
// This file in the src/metric module defines kerr_schwarzchild.h responsibilities for the
...
#ifndef PANGU_SRC_METRIC_KERRSCHWARZCHILD_H
#define PANGU_SRC_METRIC_KERRSCHWARZCHILD_H
...
void CalculatePhysicalMetric(const parthenon::Real x[4],
  ...
  gcov[0][0] = -1.0 + 2.0 * r / Sigma;
  gcov[0][1] = 2.0 * r / Sigma;
  gcov[0][3] = -2.0 * a * r * sth * sth / Sigma;

  gcov[1][0] = gcov[0][1];
  gcov[1][1] = 1.0 + 2.0 * r / Sigma;
  gcov[1][3] = -a * (1.0 + 2.0 * r / Sigma) * sth * sth;
  ...
  gcov[3][1] = gcov[1][3];
  gcov[3][3] =
      sth * sth * (Sigma + a * a * sth * sth * (1.0 + 2.0 * r / Sigma));
}
...
#endif  // PANGU_SRC_METRIC_KERRSCHWARZCHILD_H
